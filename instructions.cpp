...
/*encode methods add the following command to the array*/
...
/*The input integer should take the next 4 bytes
of mCode Also, be sure to add code to
check for buffer overflow, similar to the previous example.*/
...
void InstructionsClass::Finish()
{
	Encode(POP_EBP);
	Encode(NEAR_RET);
	MSG("Finished creating " << mCurrent << " bytes of machine code");
	// std::cout << "Finished creating " << mCurrent << " bytes of machine code" << endl;
}
...
/*which takes as input an integer parameter, and fills mCode with machine codes that will push the given integer value onto
the stack.*/
void InstructionsClass::PushValue(int value)
{
	// std::cout<<"Pushing passed value " << value << std::endl;
	/*The first byte tells the CPU to Move an integer into the EAX register,*/
	Encode(IMMEDIATE_TO_EAX);
	/*the next 4 bytes are the immediate value to copy.*/
	Encode(value);
	/*Try to determine the correct machine codes for IMMEDIATE_TO_EAX and PUSH_EAX.*/
	Encode(PUSH_EAX);
}
...
void InstructionsClass::PushVariable(unsigned int index)
{
	...
	/*The four bytes following are the RAM address 54
	of which integer to move. You can find that address by calling the helper method
	GetMem. Then write code to PUSH_EAX. In all, PushVariable should create six
...
/*PopAndStore should POP_EAX to move the integer from the top of the stack
to the EAX register. Then EAX_TO_MEM to move it from the EAX register to the
correct RAM address, as found by calling GetMem.*/
